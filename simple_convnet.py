...
class SimpleConvNet:
    ...
    def __init__(self, input_dim=(1, 28, 28),
        ...
        pool_output_size = int(filter_num * (conv_output_size/2) * (conv_output_size/2)) # 30x12x12

        # 重みの初期化
        self.params = {}
        self.params['W1'] = weight_init_std * \
                            np.random.randn(filter_num, input_dim[0], filter_size, filter_size) # filter num, channel, filter size
        self.params['b1'] = np.zeros(filter_num) # filter num
        self.params['W2'] = weight_init_std * \
                            np.random.randn(pool_output_size, hidden_size) # 144 x 30 = 4320, 100
